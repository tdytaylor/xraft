...
public class ServerLauncher {
  ...
  private void startAsGroupMember(CommandLine cmdLine) throws Exception {
    ...
    Set<NodeEndpoint> nodeEndpoints =
        Stream.of(rawGroupConfig).map(this::parseNodeEndpoint).collect(Collectors.toSet());

    Node node =
        new NodeBuilder(nodeEndpoints, new NodeId(rawNodeId))
            .setDataDir(cmdLine.getOptionValue('d'))
            .build();
    Server server = new Server(node, portService);
    logger.info(
