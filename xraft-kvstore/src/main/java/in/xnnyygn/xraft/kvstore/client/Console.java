...
public class Console {
  ...
  public Console(Map<NodeId, Address> serverMap) {
    ...
    ArgumentCompleter completer =
        new ArgumentCompleter(new StringsCompleter(commandMap.keySet()), new NullCompleter());
  ...
  private void dispatchCommand(String line) {
    ...
    command.execute(
        commandNameAndArguments.length > 1 ? commandNameAndArguments[1] : "", commandContext);
  }
}
