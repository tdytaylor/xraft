...
public class NodeBuilder {
  ...
  /**
   * Log. If data directory specified, {@link FileLog} will be created. Default to {@link
   * MemoryLog}.
   */
  private Log log = null;

  /**
   * Store for current term and last node id voted for. If data directory specified, {@link
   * FileNodeStore} will be created. Default to {@link MemoryNodeStore}.
   */
  ...
  /** Event loop group for worker. If specified, reuse. otherwise create one. */
  ...
  /**
   * Set event loop for worker. If specified, it's caller's responsibility to close worker event
   * loop.
   *
   * @param workerNioEventLoopGroup worker event loop
   * @return this
   */
  public NodeBuilder setWorkerNioEventLoopGroup(
      @Nonnull NioEventLoopGroup workerNioEventLoopGroup) {
   ...
   * @return this
   */
  NodeBuilder setGroupConfigChangeTaskExecutor(
      @Nonnull TaskExecutor groupConfigChangeTaskExecutor) {
  ...
  private NodeContext buildContext() {
    ...
    context.setTaskExecutor(
        taskExecutor != null
            ? taskExecutor
            : new ListeningTaskExecutor(
                Executors.newSingleThreadExecutor(r -> new Thread(r, "node"))));
    // TODO share monitor
    context.setGroupConfigChangeTaskExecutor(
        groupConfigChangeTaskExecutor != null
            ? groupConfigChangeTaskExecutor
            : new ListeningTaskExecutor(
                Executors.newSingleThreadExecutor(r -> new Thread(r, "group-config-change"))));
    return context;
  }
  ...
  private NioConnector createNioConnector() {
    ...
    return new NioConnector(
        new NioEventLoopGroup(config.getNioWorkerThreads()), false, selfId, eventBus, port);
  }
