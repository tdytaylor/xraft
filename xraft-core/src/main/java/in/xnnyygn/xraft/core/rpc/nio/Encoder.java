...
class Encoder extends MessageToByteEncoder<Object> {
  ...
  protected void encode(ChannelHandlerContext ctx, Object msg, ByteBuf out) throws Exception {
    if (msg instanceof NodeId) {
      this.writeMessage(
          out, MessageConstants.MSG_TYPE_NODE_ID, ((NodeId) msg).getValue().getBytes());
    } else if (msg instanceof RequestVoteRpc) {
      RequestVoteRpc rpc = (RequestVoteRpc) msg;
      Protos.RequestVoteRpc protoRpc =
          Protos.RequestVoteRpc.newBuilder()
              .setTerm(rpc.getTerm())
      ...
      Protos.RequestVoteResult protoResult =
          Protos.RequestVoteResult.newBuilder()
      ...
      Protos.AppendEntriesRpc protoRpc =
          Protos.AppendEntriesRpc.newBuilder()
      ...
      Protos.AppendEntriesResult protoResult =
          Protos.AppendEntriesResult.newBuilder()
      ...
      Protos.InstallSnapshotRpc protoRpc =
          Protos.InstallSnapshotRpc.newBuilder()
              .setTerm(rpc.getTerm())
              .setLeaderId(rpc.getLeaderId().getValue())
              .setLastIndex(rpc.getLastIndex())
              .setLastTerm(rpc.getLastTerm())
              .addAllLastConfig(
                  rpc.getLastConfig().stream()
                      .map(
                          e ->
      ...
      Protos.InstallSnapshotResult protoResult =
          Protos.InstallSnapshotResult.newBuilder().setTerm(result.getTerm()).build();
      ...
      Protos.PreVoteRpc protoRpc =
          Protos.PreVoteRpc.newBuilder()
              .setTerm(rpc.getTerm())
              .setLastLogIndex(rpc.getLastLogIndex())
              .setLastLogTerm(rpc.getLastLogTerm())
              .build();
      ...
      Protos.PreVoteResult protoResult =
          Protos.PreVoteResult.newBuilder()
              .setTerm(result.getTerm())
              .setVoteGranted(result.isVoteGranted())
              .build();
