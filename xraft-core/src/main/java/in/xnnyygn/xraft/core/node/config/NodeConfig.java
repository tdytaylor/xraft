 ...
 * Node configuration.
 *
 * <p>Node configuration should not change after initialization. e.g {@link NodeBuilder}.
 */
public class NodeConfig {
  ...
  /**
   * Interval for log replication task. More specifically, interval for heartbeat rpc. Append
   * entries rpc may be sent less than this interval. e.g after receiving append entries result from
   * followers.
   */
  private int logReplicationInterval = 1000;

  /**
   * Read timeout to receive response from follower. If no response received from follower, resend
   * log replication rpc.
   */
  ...
  /** Read timeout to receive response from new node. Default to election timeout. */
  private int newNodeReadTimeout = 3000;

  /**
   * Timeout for new node to make progress. If new node cannot make progress after this timeout, new
   * node cannot be added and reply TIMEOUT. Default to election timeout
   */
  private int newNodeAdvanceTimeout = 3000;

  /**
   * Timeout to wait for previous group config change to complete. Default is {@code 0}, forever.
   */
