...
import in.xnnyygn.xraft.core.log.snapshot.EmptySnapshot;
import in.xnnyygn.xraft.core.log.snapshot.FileSnapshot;
import in.xnnyygn.xraft.core.log.snapshot.FileSnapshotBuilder;
import in.xnnyygn.xraft.core.log.snapshot.FileSnapshotWriter;
import in.xnnyygn.xraft.core.log.snapshot.Snapshot;
import in.xnnyygn.xraft.core.log.snapshot.SnapshotBuilder;
import in.xnnyygn.xraft.core.log.snapshot.SnapshotWriter;
...
public class FileLog extends AbstractLog {
  ...
  public FileLog(File baseDir, EventBus eventBus, Set<NodeEndpoint> baseGroup) {
    ...
    if (latestGeneration != null) {
      ...
      FileEntrySequence fileEntrySequence =
          new FileEntrySequence(latestGeneration, snapshot.getLastIncludedIndex() + 1);
  ...
  protected Snapshot generateSnapshot(
    ...
    try (SnapshotWriter snapshotWriter =
        new FileSnapshotWriter(
            logDir.getSnapshotFile(),
            lastAppliedEntryMeta.getIndex(),
            lastAppliedEntryMeta.getTerm(),
            groupConfig)) {
  ...
  protected void replaceSnapshot(Snapshot newSnapshot) {
    ...
    EntrySequence newEntrySequence =
        new FileEntrySequence(fileSnapshot.getLogDir(), logIndexOffset);
  ...
  private class StateMachineContextImpl implements StateMachineContext {
    ...
    public OutputStream getOutputForGeneratingSnapshot(
        int lastIncludedIndex, int lastIncludedTerm, Set<NodeEndpoint> groupConfig)
        throws Exception {
      ...
      snapshotWriter =
          new FileSnapshotWriter(
              rootDir.getLogDirForGenerating().getSnapshotFile(),
              lastIncludedIndex,
              lastIncludedTerm,
              groupConfig);
