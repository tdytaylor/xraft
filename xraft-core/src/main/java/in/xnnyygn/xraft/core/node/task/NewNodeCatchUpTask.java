...
public class NewNodeCatchUpTask implements Callable<NewNodeCatchUpTaskResult> {
  ...
  public NewNodeCatchUpTask(
      NewNodeCatchUpTaskContext context, NodeEndpoint endpoint, NodeConfig config) {
  ...
  // in node thread
  synchronized void onReceiveAppendEntriesResult(
      AppendEntriesResultMessage resultMessage, int nextLogIndex) {
    assert nodeId.equals(resultMessage.getSourceNodeId());
    if (state != State.REPLICATING) {
      throw new IllegalStateException(
          "receive append entries result when state is not replicating");
    }
    ...
    logger.debug(
        "replication state of new node {}, next index {}, match index {}",
        nodeId,
        nextIndex,
        matchIndex);
  ...
  // in node thread
  synchronized void onReceiveInstallSnapshotResult(
      InstallSnapshotResultMessage resultMessage, int nextLogIndex) {
    assert nodeId.equals(resultMessage.getSourceNodeId());
    if (state != State.REPLICATING) {
      throw new IllegalStateException(
          "receive append entries result when state is not replicating");
