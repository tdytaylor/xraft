...
import in.xnnyygn.xraft.core.log.snapshot.NullSnapshotBuilder;
import in.xnnyygn.xraft.core.log.snapshot.Snapshot;
import in.xnnyygn.xraft.core.log.snapshot.SnapshotBuilder;
...
abstract class AbstractLog implements Log {
  ...
  public EntryMeta getLastEntryMeta() {
    if (entrySequence.isEmpty()) {
      return new EntryMeta(
          Entry.KIND_NO_OP, snapshot.getLastIncludedIndex(), snapshot.getLastIncludedTerm());
    }
    return entrySequence.getLastEntry().getMeta();
  }

  @Override
  public AppendEntriesRpc createAppendEntriesRpc(
    ...
    if (!entrySequence.isEmpty()) {
      int maxIndex =
          (maxEntries == ALL_ENTRIES
              ? nextLogIndex
              : Math.min(nextLogIndex, nextIndex + maxEntries));
  ...
  @Override
  public InstallSnapshotRpc createInstallSnapshotRpc(
      int term, NodeId selfId, int offset, int length) {
    return createInstallSnapshotRpc(term, snapshot.getLastIncludedIndex(), selfId, offset, length);
  }

  @Override
  public InstallSnapshotRpc createInstallSnapshotRpc(
      int term, int lastIncludedIndex, NodeId selfId, int offset, int length) {
  ...
  @Override
  public AddNodeEntry appendEntryForAddNode(
      int term, Set<NodeEndpoint> nodeEndpoints, NodeEndpoint newNodeEndpoint) {
    AddNodeEntry entry =
        new AddNodeEntry(entrySequence.getNextLogIndex(), term, nodeEndpoints, newNodeEndpoint);
    entrySequence.append(entry);
  ...
  @Override
  public RemoveNodeEntry appendEntryForRemoveNode(
      int term, Set<NodeEndpoint> nodeEndpoints, NodeId nodeToRemove) {
    RemoveNodeEntry entry =
        new RemoveNodeEntry(entrySequence.getNextLogIndex(), term, nodeEndpoints, nodeToRemove);
    entrySequence.append(entry);
    groupConfigEntryList.add(entry);
    return entry;
  }
  ...
  @Override
  public AppendEntriesState appendEntriesFromLeader(
      int prevLogIndex, int prevLogTerm, List<Entry> leaderEntries) {
    // check previous log
    ...
    UnmatchedLogRemovedResult unmatchedLogRemovedResult =
        removeUnmatchedLog(new EntrySequenceView(leaderEntries));
    GroupConfigEntry lastGroupConfigEntry =
        appendEntriesFromLeader(unmatchedLogRemovedResult.newEntries);
  ...
  private GroupConfigEntry appendEntriesFromLeader(EntrySequenceView leaderEntries) {
    ...
    logger.debug(
        ...
        leaderEntries.getFirstLogIndex(),
        leaderEntries.getLastLogIndex());
  ...
  private boolean checkIfPreviousLogMatches(int prevLogIndex, int prevLogTerm) {
    ...
    if (prevLogIndex < lastIncludedIndex) {
      logger.debug(
          "previous log index {} < snapshot's last included index {}",
          prevLogIndex,
          lastIncludedIndex);
      return false;
    }
  ...
  private GroupConfigEntry removeEntriesAfter(int index) {
    ...
    if (index < lastApplied
        && entrySequence.subList(index + 1, lastApplied + 1).stream()
  ...
  public List<GroupConfigEntry> advanceCommitIndex(int newCommitIndex, int currentTerm) {
    ...
    List<GroupConfigEntry> groupConfigEntries =
        groupConfigEntryList.subList(commitIndex + 1, newCommitIndex + 1);
    logger.debug(
        "group configs from {} to {}, {}", commitIndex + 1, newCommitIndex + 1, groupConfigEntries);
  ...
  private void applyEntry(Entry entry) {
    ...
    if (isApplicable(entry)) {
      Set<NodeEndpoint> lastGroup =
          groupConfigEntryList.getLastGroupBeforeOrDefault(entry.getIndex());
      stateMachine.applyLog(
          stateMachineContext,
          entry.getIndex(),
          entry.getTerm(),
          entry.getCommandBytes(),
          entrySequence.getFirstLogIndex(),
          lastGroup);
    } else {
  ...
  //        for (GroupConfigEntry groupConfigEntry : groupConfigEntryList.subList(commitIndex + 1,
  // newCommitIndex + 1)) {
  ...
  private boolean validateNewCommitIndex(int newCommitIndex, int currentTerm) {
    ...
    if (entry.getTerm() != currentTerm) {
      logger.debug(
          "log term of new commit index != current term ({} != {})", entry.getTerm(), currentTerm);
      return false;
    }
    return true;
  }
  ...
  protected abstract Snapshot generateSnapshot(
      EntryMeta lastAppliedEntryMeta, Set<NodeEndpoint> groupConfig);
  ...
  public InstallSnapshotState installSnapshot(InstallSnapshotRpc rpc) {
    ...
    return new InstallSnapshotState(
        InstallSnapshotState.StateName.INSTALLED, newSnapshot.getLastConfig());
  }
