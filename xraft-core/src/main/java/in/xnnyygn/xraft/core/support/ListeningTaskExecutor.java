...
public class ListeningTaskExecutor extends AbstractTaskExecutor {
  ...
  public ListeningTaskExecutor(ListeningExecutorService listeningExecutorService) {
    this(
        ...
        Executors.newSingleThreadExecutor(r -> new Thread(r, "monitor")),
        false);
  }

  public ListeningTaskExecutor(
      ExecutorService executorService, ExecutorService monitorExecutorService) {
    this(MoreExecutors.listeningDecorator(executorService), monitorExecutorService, true);
  }

  private ListeningTaskExecutor(
      ListeningExecutorService listeningExecutorService,
      ExecutorService monitorExecutorService,
      boolean monitorShared) {
  ...
  @Override
  public void submit(
      @Nonnull Runnable task, @Nonnull Collection<FutureCallback<Object>> callbacks) {
    Preconditions.checkNotNull(task);
