...
import in.xnnyygn.xraft.core.node.task.AddNodeTask;
import in.xnnyygn.xraft.core.node.task.FixedResultGroupConfigTaskReference;
import in.xnnyygn.xraft.core.node.task.FutureGroupConfigChangeTaskReference;
import in.xnnyygn.xraft.core.node.task.GroupConfigChangeTask;
import in.xnnyygn.xraft.core.node.task.GroupConfigChangeTaskContext;
...
import in.xnnyygn.xraft.core.node.task.NewNodeCatchUpTask;
import in.xnnyygn.xraft.core.node.task.NewNodeCatchUpTaskContext;
import in.xnnyygn.xraft.core.node.task.NewNodeCatchUpTaskGroup;
import in.xnnyygn.xraft.core.node.task.NewNodeCatchUpTaskResult;
import in.xnnyygn.xraft.core.node.task.RemoveNodeTask;
...
public class NodeImpl implements Node {
  ...
  private static final FutureCallback<Object> LOGGING_FUTURE_CALLBACK =
  ...
  private final NewNodeCatchUpTaskContext newNodeCatchUpTaskContext =
      new NewNodeCatchUpTaskContextImpl();
  private final NewNodeCatchUpTaskGroup newNodeCatchUpTaskGroup = new NewNodeCatchUpTaskGroup();
  private final GroupConfigChangeTaskContext groupConfigChangeTaskContext =
      new GroupConfigChangeTaskContextImpl();
  ...
  public synchronized void start() {
    ...
    changeToRole(
        new FollowerNodeRole(
            store.getTerm(), store.getVotedFor(), null, 0, 0, scheduleElectionTimeout()));
    started = true;
  }
  ...
  public GroupConfigChangeTaskReference addNode(@Nonnull NodeEndpoint endpoint) {
    ...
    NewNodeCatchUpTask newNodeCatchUpTask =
        new NewNodeCatchUpTask(newNodeCatchUpTaskContext, endpoint, context.config());
    ...
    try {
      ...
      switch (newNodeCatchUpTaskResult.getState()) {
        case REPLICATION_FAILED:
          return new FixedResultGroupConfigTaskReference(
    ...
    synchronized (this) {
      ...
      currentGroupConfigChangeTask =
          new AddNodeTask(groupConfigChangeTaskContext, endpoint, newNodeCatchUpTaskResult);
      Future<GroupConfigChangeTaskResult> future =
          context.groupConfigChangeTaskExecutor().submit(currentGroupConfigChangeTask);
  ...
  private GroupConfigChangeTaskResult awaitPreviousGroupConfigChangeTask() {
    try {
      currentGroupConfigChangeTaskReference.awaitDone(
  ...
  private void ensureLeader() {
    ...
    NodeEndpoint endpoint =
        result.getLeaderId() != null
            ? context.group().findMember(result.getLeaderId()).getEndpoint()
            : null;
  ...
  public GroupConfigChangeTaskReference removeNode(@Nonnull NodeId id) {
    ...
    synchronized (this) {
      ...
      currentGroupConfigChangeTask =
          new RemoveNodeTask(groupConfigChangeTaskContext, id, context.selfId());
      Future<GroupConfigChangeTaskResult> future =
          context.groupConfigChangeTaskExecutor().submit(currentGroupConfigChangeTask);
      currentGroupConfigChangeTaskReference = new FutureGroupConfigChangeTaskReference(future);
      return currentGroupConfigChangeTaskReference;
  ...
  synchronized void cancelGroupConfigChangeTask() {
    ...
    currentGroupConfigChangeTaskReference =
        new FixedResultGroupConfigTaskReference(GroupConfigChangeTaskResult.OK);
  }
  ...
  private void doProcessElectionTimeout() {
    ...
    if (context.group().isStandalone()) {
      ...
      if (role.getName() == RoleName.FOLLOWER) {
        // pre-vote
        changeToRole(
            new FollowerNodeRole(role.getTerm(), null, null, 1, 0, scheduleElectionTimeout()));
   ...
   *
   * <p>It is stable when role name not changed and role state except timeout/task not change.
   *
   * <p>If role state except timeout/task not changed, it should not update store or notify
   * listeners.
   *
   ...
   *
   * <p>Normally it will send append entries rpc to node. And change to install snapshot rpc if
   * entry in snapshot.
   *
   * @param member node
  ...
  private void doReplicateLog(GroupMember member, int maxEntries) {
    ...
    try {
      ...
      logger.debug(
          "log entry {} in snapshot, replicate with install snapshot RPC", member.getNextIndex());
      InstallSnapshotRpc rpc =
          context
              .log()
              .createInstallSnapshotRpc(
                  role.getTerm(), context.selfId(), 0, context.config().getSnapshotDataLength());
  ...
  private PreVoteResult doProcessPreVoteRpc(PreVoteRpcMessage rpcMessage) {
    ...
    return new PreVoteResult(
        role.getTerm(), !context.log().isNewerThan(rpc.getLastLogIndex(), rpc.getLastLogTerm()));
  }
  ...
  private void doProcessPreVoteResult(PreVoteResult result) {
    ...
    if (currentPreVotesCount > countOfMajor / 2) {
      ...
      changeToRole(
          new FollowerNodeRole(
              role.getTerm(), null, null, currentPreVotesCount, 0, scheduleElectionTimeout()));
    }
  }
  ...
  private RequestVoteResult doProcessRequestVoteRpc(RequestVoteRpcMessage rpcMessage) {
    ...
    //            logger.warn("receive request vote rpc from node {} which is not major node,
    // ignore", rpcMessage.getSourceNodeId());
    //            return new RequestVoteResult(role.getTerm(), false);
    //        }

    if (role.getName() == RoleName.FOLLOWER
        && (System.currentTimeMillis() - ((FollowerNodeRole) role).getLastHeartbeat()
            < context.config().getMinElectionTimeout())) {
      return null;
    }
    ...
    if (rpc.getTerm() < role.getTerm()) {
      logger.debug(
          "term from rpc < current term, don't vote ({} < {})", rpc.getTerm(), role.getTerm());
      return new RequestVoteResult(role.getTerm(), false);
    }

    // step down if result's term is larger than current term
    if (rpc.getTerm() > role.getTerm()) {
      boolean voteForCandidate =
          !context.log().isNewerThan(rpc.getLastLogIndex(), rpc.getLastLogTerm());
      becomeFollower(
          rpc.getTerm(), (voteForCandidate ? rpc.getCandidateId() : null), null, 0, true);
  ...
  public void onReceiveRequestVoteResult(RequestVoteResult result) {
    context
        .taskExecutor()
        .submit(() -> doProcessRequestVoteResult(result), LOGGING_FUTURE_CALLBACK);
  }
  ...
  private void doProcessRequestVoteResult(RequestVoteResult result) {
    ...
    if (currentVotesCount > countOfMajor / 2) {
      ...
      // update votes count
      changeToRole(
          new CandidateNodeRole(role.getTerm(), currentVotesCount, scheduleElectionTimeout()));
    }
  }
    ...
    context
        ...
        .submit(
            () ->
                context
                    .connector()
                    .replyAppendEntries(doProcessAppendEntriesRpc(rpcMessage), rpcMessage),
            LOGGING_FUTURE_CALLBACK);
  }
  ...
  private AppendEntriesResult doProcessAppendEntriesRpc(AppendEntriesRpcMessage rpcMessage) {
    ...
    switch (role.getName()) {
      case FOLLOWER:
        ...
        becomeFollower(
            rpc.getTerm(),
            ((FollowerNodeRole) role).getVotedFor(),
            rpc.getLeaderId(),
  ...
  public void onReceiveAppendEntriesResult(AppendEntriesResultMessage resultMessage) {
    context
        .taskExecutor()
        .submit(() -> doProcessAppendEntriesResult(resultMessage), LOGGING_FUTURE_CALLBACK);
  }
  ...
  private void doProcessAppendEntriesResult(AppendEntriesResultMessage resultMessage) {
    ...
    if (newNodeCatchUpTaskGroup.onReceiveAppendEntriesResult(
        resultMessage, context.log().getNextIndex())) {
      return;
    }
    ...
    if (member == null) {
      logger.info(
          "unexpected append entries result from node {}, node maybe removed", sourceNodeId);
      return;
    }
    ...
    if (result.isSuccess()) {
      ...
      if (member.advanceReplicatingState(rpc.getLastEntryIndex())) {
        List<GroupConfigEntry> groupConfigEntries =
            context
                .log()
                .advanceCommitIndex(context.group().getMatchIndexOfMajor(), role.getTerm());
    ...
    context
        ...
        .submit(
            () ->
                context
                    .connector()
                    .replyInstallSnapshot(doProcessInstallSnapshotRpc(rpcMessage), rpcMessage),
            LOGGING_FUTURE_CALLBACK);
  }
  ...
  public void onReceiveInstallSnapshotResult(InstallSnapshotResultMessage resultMessage) {
    context
        .taskExecutor()
        .submit(() -> doProcessInstallSnapshotResult(resultMessage), LOGGING_FUTURE_CALLBACK);
  }
  ...
  private void doProcessInstallSnapshotResult(InstallSnapshotResultMessage resultMessage) {
    ...
    // dispatch to new node catch up task by node id
    if (newNodeCatchUpTaskGroup.onReceiveInstallSnapshotResult(
        resultMessage, context.log().getNextIndex())) {
      return;
    }
    ...
    if (member == null) {
      logger.info(
          "unexpected install snapshot result from node {}, node maybe removed", sourceNodeId);
      return;
    }
      ...
      InstallSnapshotRpc nextRpc =
          context
              ...
              .createInstallSnapshotRpc(
                  ...
                  rpc.getLastIndex(),
                  context.selfId(),
                  rpc.getOffset() + rpc.getDataLength(),
                  context.config().getSnapshotDataLength());
      ...
      context
          ...
          .submit(
              () -> {
                ...
                AddNodeEntry entry =
                    context.log().appendEntryForAddNode(role.getTerm(), nodeEndpoints, endpoint);
