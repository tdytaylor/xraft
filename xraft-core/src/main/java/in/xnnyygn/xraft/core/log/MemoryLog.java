...
import in.xnnyygn.xraft.core.log.snapshot.MemorySnapshotBuilder;
...
public class MemoryLog extends AbstractLog {
  ...
  public MemoryLog(
      ...
      EventBus eventBus,
      Set<NodeEndpoint> initialGroup) {
    super(eventBus);
    setStateMachineContext(stateMachineContext);
    this.snapshot = snapshot;
    this.entrySequence = entrySequence;
    Set<NodeEndpoint> lastGroup = snapshot.getLastConfig();
    this.groupConfigEntryList =
        new GroupConfigEntryList((lastGroup.isEmpty() ? initialGroup : lastGroup));
  }
  ...
  private class StateMachineContextImpl implements StateMachineContext {
    ...
    MemorySnapshot buildSnapshot() {
      return new MemorySnapshot(
          lastIncludedIndex, lastIncludedTerm, output.toByteArray(), groupConfig);
    }
  }
}
